...
#include "muscle_model_plugin/thelen2003muscle.h"
...
void Thelen2003Muscle::computeInitialFiberEquilibrium(SimTK::State& s) const
{
	...
	try {
		 result = initMuscleState(0,0, activation, tol, maxIter);
	}
...
Thelen2003Muscle::initMuscleState(const double fiberLength,
                                  const double lengtheningSpeed,
                                  ...
                                  const int aMaxIterations) const
{
	...
	const double ml = fiberLength;
	const double dml= lengtheningSpeed;
